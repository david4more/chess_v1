...
void Game::initVarsAndConsts()
{
	...
	*/
	...
	if (piecesType)
		...
		piecesTexture.loadFromFile("../../../../files/Chess_Pieces_Sprite_2.png");
...
Game::Game()
{
	...
	piecesType = rand() % 2;	// 0 - 1
	boardType = rand() % 4;		// 0 - 3

	initWindow();
	initVarsAndConsts();
	boardInitializingWithAnimation();
	squares[0][0].setFillColor(black);		// In order to not to check, if it is the first moove every time i render,  just show the [0][0] square before the first moove and then set fill color
	initPieces();
}
...
void Game::initWindow()
{
	...
	window = new sf::RenderWindow(videoMode, "Chess");
